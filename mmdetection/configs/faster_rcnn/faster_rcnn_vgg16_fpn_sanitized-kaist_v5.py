...
# classes = ['person', 'people', 'person?', 'cyclist', 'person?a']
...
model = dict(
    ...
    taf_cfg=dict(
        ...
        neg_entropy_weight=0.2),   # best 1.0
    ...
    rpn_head=dict(
        ...
        anchor_generator=dict(
            ...
            strides=[8, 16],  # 在不同的FPN尺度上生成multi_level_anchors, 每一尺度面积为 (scales * strides)^2
        ),
        bbox_coder=dict(
    ...
    test_cfg=dict(
        rpn=dict(
            nms_pre=3000,       # default 1000
            max_per_img=1000,   # default 1000
            nms=dict(type='nms', iou_threshold=0.9),    # default 0.7
            min_bbox_size=0),  # default 0
        rcnn=dict(
            score_thr=0.01,  # default 0.1
            nms=dict(type='nms', iou_threshold=0.45),   # best 0.45
